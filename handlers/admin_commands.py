...
async def admin_panel(callback: CallbackQuery):
    ...
    await callback.message.answer(f"""Выберите нужеый вариант""", reply_markup = keyboard.callback_buttons(["Очистить базу данных", "Добавить пользователя", "Бан пользователя"],["clear_db", "add_user", "ban_user"]))
...
async def edit_ancet_delete(callback: CallbackQuery, state: FSMContext):
    Users.cursor.execute("UPDATE users SET username = 'unknown', age = 0, gender = 'unknown', connect = 'none', microphone = 'unknown', description = 'none', games = 'none' WHERE uid = ?", (callback.from_user.id,))
    db.save()
...
async def ban_days_handler(message: Message, state: FSMContext, bot: Bot):
    ...
    Users.cursor.execute(f"UPDATE users SET ban_days = ? WHERE uid = ?", (message.text, ban_dict["ban"]['uid']))
    db.save()
    ban_time_dict[ban_dict["ban"]['uid']] = asyncio.create_task(unban_user(Users,ban_time_dict, bot, ban_dict["ban"]['uid']))
